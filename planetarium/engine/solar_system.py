...
class Planet:
    def __init__ (self, basicOrbitElements, extraOrbitElements, period, radius, color):
    ...
    def getOrbit (self, startDateTime, orbitSteps = 180):
        ...
        t_0 = (ut.julianDayNr (startDateTime) - ut.julianDayNr (dt.datetime (2000, 1, 1, 0, 0, 0)))
        ...
        for i in range (orbitSteps):
            ...
            orbit.append  (ut.equatFromEclipt (
...
class SolarSystem:
    def __init__ (self):
        self.planets = {
            'mercury': Planet (
                ...
                (0, 0, 0, 0),# Apply rotation to bring particular planet into view
                88,
                2440,
                #lightGray
            ),
            ...
            'uranus': Planet (
                ...
                (0.00058331, -0.97731848, 0.16789245, 7.67025000),
                30660,
                25266,
                #green
            ),
    ...
    def getOrbit (self, planetName, startDateTime):
        return self.planets [planetName] .getOrbit (startDateTime)
    
    def getOrbits (self, startDateTime):
        __pragma__ ('iconv')
        return [self.getOrbit (planetName, startDateTime) for planetName in self.planets]
        __pragma__ ('noiconv')

    def getPosition (self, planetName, startDateTime):
        return self.getOrbit (planetName, startDateTime, 1)[0]

    def getPositions (self, startDateTime):
        __pragma__ ('iconv')
        return [self.getPosition (planetName, startDateTime) for planetName in self.planets]
        __pragma__ ('noiconv')
